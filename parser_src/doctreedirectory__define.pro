...
function doctreedirectory::init, location=location, files=files, system=system
  ...
  self.rstFiles = obj_new('MGcoArrayList', type=11, block_size=4)
  ...
  for f = 0L, n_elements(files) - 1L do begin
    ...
    case strlowcase(extension) of
      ...
      'idldoc': begin
        ...
        'rst': begin
          rstFileParser = self.system->getParser('rstfile')
          file = rstFileParser->parse(root + files[f], directory=self)
          self.rstFiles->add, file
        end
    endcase
  endfor
